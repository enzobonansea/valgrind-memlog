#!/usr/bin/env bash
# Snapshot of plow progress: completed result.csvs out of 34, running query,
# current spill, free disk, free memory.
set -u
...
disk=$(df -h / | awk 'NR==2 {print "free="$4" used="$3"/"$2}')
...
echo
echo "Last log lines:"
grep -E '^>>> |row\(s\) in|FAILED|ALL DONE|watchdog' "$PLOW_LOG" 2>/dev/null | tail -8

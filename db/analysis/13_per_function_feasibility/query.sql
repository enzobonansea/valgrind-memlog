...
-- ld-2., dl-init, ???.
--
-- Performance: aggregate trailing-zero counts per (alloc_type, alloc_stack)
-- in a single scan (DuckDB groups on the dictionary index of alloc_stack
-- so this is ~free), then extract the site name from each unique stack
-- once and re-aggregate by site. Per-bench iteration so each invocation
-- only touches one parquet's stack dictionary — running over all_stores
-- in one shot blew the 1h watchdog.
--
...
WITH per_stack AS (
    ...
    GROUP BY alloc_type, alloc_stack
),
sited AS (
    SELECT *,
        regexp_extract(

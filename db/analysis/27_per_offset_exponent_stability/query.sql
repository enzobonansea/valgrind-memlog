...
-- NeurIPS (HFP8).
WITH ex AS (
    SELECT bench, alloc_stack, alloc_type, alloc_addr, generation, "offset",
        CASE alloc_type
...
per_offset AS (
    ...
    GROUP BY bench, alloc_stack, alloc_type, alloc_addr, generation, "offset"
    HAVING COUNT(*) >= 2
),
per_stack AS (
...
SELECT bench, alloc_type, site, distinct_offsets,
       frac_constant_exp, mean_exp_range, median_exp_range, p95_exp_range
FROM (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY bench ORDER BY distinct_offsets DESC) AS rnk
    FROM agg
)
WHERE rnk <= 20
ORDER BY bench, distinct_offsets DESC;
